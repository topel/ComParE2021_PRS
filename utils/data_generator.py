...
class AudioSetDataset(object):
    def __init__(self, sample_rate=32000, use_audeep=False, use_one_hot=False, do_binary_task=False):
        """This class takes the meta of an audio clip as input, and return
        the waveform and target of the audio clip. This class is used by DataLoader.
        """
        self.sample_rate = sample_rate
        self.use_audeep=use_audeep
        self.use_one_hot = use_one_hot
        self.do_binary_task = do_binary_task
        # if do_binary_task:
        #     self.conversion_matrix = torch.as_tensor(np.array([[1.,0.], [0., 1.], [0., 1.], [0., 1.], [0., 1.]]))
    ...
    def __getitem__(self, meta):
        ...
        has_labels = 'train' in hdf5_path or 'devel' in hdf5_path
        ...
        with h5py.File(hdf5_path, 'r') as hf:
            ...
            if has_labels:
                if self.use_one_hot:
                    ...
                    # print("DATA GEN before:", type(target), target)
                    if self.do_binary_task:
                        target = np.array([target[0], np.sum(target[1:])]).astype(np.float32)
                        # target = np.array(np.sum(target[1:])).astype(np.float32)
                        # print("DATA GEN after:", type(target), target)
                else:
                    # if CE
                    target = self.onehot_to_long(hf['target'][index_in_hdf5].astype(np.long))
                    # print("data_gen", target)

                    if self.do_binary_task:
                        target = 1.*(target>0) # type float
                        # target = 1 * (target > 0) # type long
                # print("target", type(target), target)

            if self.use_audeep:
                audeep = hf['audeep'][index_in_hdf5]

        # data_dict = {
        #     'audio_name': audio_name, 'waveform': waveform, 'target': target, 'target2': target2}

        if has_labels:
            if self.use_audeep:
                data_dict = {
                    'audio_name': audio_name, 'waveform': waveform, 'target': target, 'audeep': audeep}
            else:
                data_dict = {
            ...
            if self.use_audeep:
                data_dict = {
                    'audio_name': audio_name, 'waveform': waveform, 'audeep': audeep}
            else:
                data_dict = {
                    'audio_name': audio_name, 'waveform': waveform}

        return data_dict

    def onehot_to_long(self, t):
        ind = np.arange(5)
        return ind[t>0.5]
...
class EvaluateSampler(object):
    def __init__(self, indexes_hdf5_path, batch_size):
        ...
        """
        self.batch_size = batch_size
        self.has_targets = 'test' not in indexes_hdf5_path
