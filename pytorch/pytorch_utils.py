...
def forward(model, generator, use_audeep=False, return_input=False,
            return_target=False):
    ...
    for n, batch_data_dict in enumerate(generator):
        ...
        if use_audeep:
            batch_audeep = move_data_to_device(batch_data_dict['audeep'], device)

        with torch.no_grad():
            model.eval()
            if use_audeep:
                batch_output = model(batch_waveform, batch_audeep)
            else:
                batch_output = model(batch_waveform)

        append_to_dict(output_dict, 'audio_name', batch_data_dict['audio_name'])

        append_to_dict(output_dict, 'clipwise_output_tensor',
                       batch_output['clipwise_output'].data.cpu())

        append_to_dict(output_dict, 'clipwise_output',
                       batch_output['clipwise_output'].data.cpu().numpy())

        append_to_dict(output_dict, 'embedding',
                       batch_output['embedding'].data.cpu().numpy())
        ...
        if n % 100 == 0:
            print(' --- Inference time: {:.3f} s / 100 iterations ---'.format(
                time.time() - time1))
            time1 = time.time()
